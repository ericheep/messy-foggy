...
function setFixtures() {
    fixtures = arrayfromargs(arguments)
    numFixtures = fixtures.length    
}

function setLedNums() {
    ledNums = arrayfromargs(arguments)
    
    totalLeds = 0
    for (var i = 0; i < ledNums.length; i++) {
        totalLeds += ledNums[i];
    }
...
function getBrickLedIndex(index) {
   for (var i = 0; i < totalLeds; i++) {
        let runningNum = 0
        let runningLedNum = 0
...
function init() {    
    ...
    for (var i = 0; i < totalLeds; i++) {        
        ...
        let led = {
                "brickIndex": getBrickIndex(i),
                "brickLedIndex": getBrickLedIndex(i),
                "index": i,
                "r": rgb.r,
                "g": rgb.g,
                "b": rgb.b,
...
function outputLed(led) {
    let s = led.s * sat
    let v = clamp((led.v + globalFade) * bri, 0.0, 1.0)
        
    //let rgb = color.HSVtoRGB(led.h, s, v)
    //outlet(0, [led.brickIndex, led.brickLedIndex, rgb.r, rgb.g, rgb.b])   
    outlet(0, [led.brickIndex, led.brickLedIndex, led.h, led.s, led.v])   
}
